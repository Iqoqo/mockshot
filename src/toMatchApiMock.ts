...
async function parse(response) {
  if (response.config && response.config.method && response.config.url && response.status && response.data) {
    ...
    return { method: "unknown Method", url: "unknown URL", mock: { status: -1, body: undefined } };
  }
}
...
async function toMatchApiMock(
  ...
  const snapshotNameTag = `${parsedResponse.method} ${parsedResponse.url} ${mockName}`;
